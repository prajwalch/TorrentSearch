...
/**
 * Taken from [mihon](https://github.com/mihonapp/mihon/blob/main/core/common/src/main/kotlin/eu/kanade/tachiyomi/network/DohProviders.kt)
 */
object DohProviders {
    fun buildCloudflare(bootstrapClient: OkHttpClient) =
    ...
    fun buildGoogle(bootstrapClient: OkHttpClient) =
        DnsOverHttps.Builder().client(bootstrapClient)
    ...
    fun buildNextDNS(bootstrapClient: OkHttpClient) =
        DnsOverHttps.Builder().client(bootstrapClient)
            .url("https://dns.nextdns.io/cfce9b".toHttpUrl())
            .bootstrapDnsHosts(
                InetAddress.getByName("45.90.28.192"),
                InetAddress.getByName("45.90.30.193"),
                InetAddress.getByName("2a07:a8c0::cf:ce9b"),
                InetAddress.getByName("2a07:a8c1::cf:ce9b"),
            )
            .build()

    // AdGuard "Default" DNS works too but for the sake of making sure no site is blacklisted,
    // we use "Unfiltered"
    fun buildAdGuard(bootstrapClient: OkHttpClient) =
        DnsOverHttps.Builder().client(bootstrapClient)
    ...
    fun buildQuad9(bootstrapClient: OkHttpClient) =
        DnsOverHttps.Builder().client(bootstrapClient)
    ...
    fun buildAliDNS(bootstrapClient: OkHttpClient) =
        DnsOverHttps.Builder().client(bootstrapClient)
    ...
    fun buildDNSPod(bootstrapClient: OkHttpClient) =
        DnsOverHttps.Builder().client(bootstrapClient)
    ...
    fun build360(bootstrapClient: OkHttpClient) =
        DnsOverHttps.Builder().client(bootstrapClient)
    ...
    fun buildQuad101(bootstrapClient: OkHttpClient) =
        DnsOverHttps.Builder().client(bootstrapClient)
    ...
    /**
     * Mullvad DoH without ad blocking option.
     * Source: https://mullvad.net/en/help/dns-over-https-and-dns-over-tls
     */
    fun buildMullvad(bootstrapClient: OkHttpClient) =
        DnsOverHttps.Builder().client(bootstrapClient)
    ...
    /**
     * Control D unfiltered option.
     * Source: https://controld.com/free-dns/?
     */
    fun buildControlD(bootstrapClient: OkHttpClient) =
        DnsOverHttps.Builder().client(bootstrapClient)
     ...
     */
    fun buildNajalla(bootstrapClient: OkHttpClient) =
        DnsOverHttps.Builder().client(bootstrapClient)
     ...
     */
    fun buildShecan(bootstrapClient: OkHttpClient) =
        DnsOverHttps.Builder().client(bootstrapClient)
